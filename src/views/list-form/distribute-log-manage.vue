...
import { getDistributionLogList as get } from '@/config/api';
import { DistributeLogColumns, PageAuth } from '@/base-class/list';
import { distributeLogCondition as searchCondition } from '@/config/conditions';
import { userModule } from '@/store/index';

type PageType = 'DistributionLogList';
...
export default class ConfigManage extends DistributeLogColumns {
    // 当前页面是否以弹出形式打开
    @Prop(Boolean)
    pageType!: boolean;
    // 外部传递的请求参数参数
    ...
    authKeyChange(val: string | undefined) {
        this.getAuth(val as 'distribute-log-manage');
    }
    /**
    ...
    // 列表操作栏点击事件
    async handle(name: PageAuth['distribute-log-manage'], data?: API.Response['DistributionLogList']) {

        switch (name) {
            case 'view':
                this.$getDynamicComponent('configManage', () => {
                    this.$createConfigManageHandle({
                        type: 'details',
                        data,
                        $events: {
                            success: 'refresh',
                ...
                break;
            case 'viewDistributeResult':
                this.$getDynamicComponent('distributeResult', () => {
                    this.$createDistributeResultHandle({
                        id: data && data.id,
                        $events: {
                            success: 'refresh',
                        },

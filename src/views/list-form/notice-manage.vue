...
import { getNoticeList as getList, deleteNotice as del } from '@/config/api';
import { NoticeColumns, PageAuth } from '@/base-class/list';
import { noticeCondition as searchCondition } from '@/config/conditions';

type PageType = 'NoticeList';
...
export default class NoticeManage extends NoticeColumns {
    // 当前页面是否以弹出形式打开
    @Prop(Boolean)
    pageType!: boolean;
    // 外部传递的请求参数参数
    ...
    authKeyChange(val: string | undefined) {
        this.getAuth(val as 'notice-manage');
    }
    /**
    ...
    // 列表操作栏点击事件
    async handle(name: PageAuth['notice-manage'], data?: API.Response['NoticeInfo']) {
        switch (name) {
            case 'add':
            case 'edit':
            case 'details':
                this.$getDynamicComponent('noticeManage', () => {
                    this.$createNoticeManageHandle({

...
import { getConfigList as get, delectConfig as del } from '@/config/api';
import { ConfigColumns, PageAuth } from '@/base-class/list';
import { configCondition as searchCondition } from '@/config/conditions';
import { userModule } from '@/store/index';

type PageType = 'ConfigList';
...
export default class ConfigManage extends ConfigColumns {
    // 当前页面是否以弹出形式打开
    @Prop(Boolean)
    pageType!: boolean;
    // 外部传递的请求参数参数
    ...
    authKeyChange(val: string | undefined) {
        this.getAuth(val as 'config-manage');
    }
    /**
    ...
    // 列表操作栏点击事件
    async handle(name: PageAuth['config-manage'], data?: API.Response['ConfigList']) {
        const { orgs } = this;

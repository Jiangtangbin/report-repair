<template>
    <div class="modify-wrapper">
        <Steps :current="current">
            ...
            <Step :title="$t('h.tips.getSmsCode') + $t('h.formLabel.fresh')"></Step>
        ...
        <i-form :model="formInline" :rules="rules" class="form" ref="form">
            ...
            <form-item v-show="current !== 0" :label="`${$t('h.formLabel.fresh')}: `" :rules="current !== 0 ? undefined : {}" prop="new">
                <i-input v-model="formInline.new" :readonly="!(extra.code && formInline.verify)" :placeholder="$t('h.placeholder.pleaseEnter', { msg: $t('h.formLabel.fresh') })" type="password" />
            </form-item>
            <form-item v-show="current !== 0" :label="`${$t('h.formLabel.confirmFresh')}: `" :rules="current !== 0 ? undefined : {}" prop="confirmNew">
                <i-input v-model="formInline.confirmNew" :readonly="!(extra.code && formInline.verify)" :placeholder="$t('h.placeholder.pleaseEnter', { msg: $t('h.formLabel.confirmFresh') })" type="password" />
            </form-item>
            <form-item v-if="current === 2" :label="`${$t('h.tips.againLogin')}`"></form-item>
        </i-form>
...
export default class Personal extends Vue {
    ...
    get rules() {
        return {
            ...
            new: { required: true, validator: (rule: RegExp, value: string, callback: Function) => callback(passwordReg(value, true)), trigger: 'blur' },
            confirmNew: { required: true, validator: (rule: RegExp, value: string, callback: Function) => callback(passwordReg(value, true)), trigger: 'blur' },
        };
    }
    ...
    async next() {
        const { $refs: { form: { validate }}, formInline: { new: newP, confirmNew, ...args }} = this;
        ...
        if (flag) {
            if (this.current === 0) {
                ...
                if (newP === confirmNew) {
                    ...
                    this.$Message.error(this.$t('h.tips.confirmAgreement'));
                }
            }
        }
    }
